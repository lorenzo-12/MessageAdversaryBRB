...
QU\ANTAS is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.
...
namespace quantas{
    ...
    public:
        ...
        ~Network                                                ();
        map<string,vector<int>> term_time;
        vector<bool> byz_vec;
        long sender_id;
        ...
        vector<Peer<type_msg>*>             peers               ()                                              {return _peers;}
	...
	void Network<type_msg, peer_type>::initNetwork(json topology, int lastRound) {
        ...
        // number of nodes that will be broadcasted by the sender
        int msgToSend = topology["msgToSend"];

        // for each message that will be sent create create the tuple (msgId, deliveryVector)
        for (int i=0; i<msgToSend; i++){
            // reset the vector (so that it will not contains value from previous tests)
            term_time[std::to_string(i)].clear();
            // fill the vector of "-1"
            for (int j=0; j<topology["totalPeers"]; j++){
                term_time[std::to_string(i)].push_back(-1);
            }
        }
        ...
        // if we are considering an experiment with byzantine nodes then execute this
        if (topology.contains("byzantine")){
            int n = topology["totalPeers"];
            vector<long> byz_list = topology["byzantine"]["list"];
            int t = topology["byzantine"]["total"];
            for (int i=0; i<n; i++) byz_vec.push_back(false);
            for (auto i : byz_list) byz_vec[i] = true;

            for (int i = 0; i < topology["totalPeers"]; i++) {
                _peers[i]->initParameters(_peers, topology, term_time);
            }
        }
    ...
    void Network<type_msg, peer_type>::initParameters(json parameters) {
        //cout << "[Network.hpp] initParameters" << endl;
