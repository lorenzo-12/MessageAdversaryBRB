...
namespace quantas{
    ...
    private:
        ...
        string                                          _ma_type;
        int                                             _ma_power; 
        vector<interfaceId>                             _nodes_disconnected;
    ...
    public: 
        ...
        NetworkInterface                                         (interfaceId id, string ma_type, int ma_power, vector<interfaceId> _nodes_disconnected = {});
        ~NetworkInterface                                        (){};
        // Setters
        ...
        void                               setMA                 (int total_nodes, const string type, int power, vector<interfaceId> nodes_disconnected)   
                                                                                                            {_total_nodes = total_nodes; _ma_type  = type; _ma_power = power; _nodes_disconnected=nodes_disconnected; }
    ...
    template <class message>
    int NetworkInterface<message>::broadcast(message msg, vector<long> node_list){
    ...
    int NetworkInterface<message>::broadcast_MA1(message msg, vector<long> excluded){
        ...
        std::sample(
            remainingNeighbors.begin(), remainingNeighbors.end(),
            std::back_inserter(blocked),
            std::min<int>(_ma_power, remainingNeighbors.size()),
            RANDOM_GENERATOR
        );

        // remove the peers from the blocked list
        for (auto peer : blocked){
            remainingNeighbors.erase(std::remove(remainingNeighbors.begin(), remainingNeighbors.end(), peer), remainingNeighbors.end()); 
        }

        // this are the message that the nodes sends (even if some of them will be blocked by the MA)
        int messagesSent = broadcast(msg, remainingNeighbors) + _ma_power;
        return messagesSent;
    }
    ...
    template <class message>
    NetworkInterface<message>::NetworkInterface(interfaceId id, string ma_type, int ma_power, vector<interfaceId> nodes_disconnected){
        _id = id;
        _inStream = deque<Packet<message> >();
        _outStream = deque<Packet<message> >();
        _outBoundChannels = map<interfaceId, NetworkInterface<message>* >();
        ...
        _ma_type = ma_type;
        _ma_power = ma_power;
        _nodes_disconnected = nodes_disconnected;
    }
